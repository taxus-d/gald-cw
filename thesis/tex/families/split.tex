...
\begin{figure}[htpb]
  \centering
  \includepgfgraphics[width=0.77\linewidth]{img/families/bulge1.5c/discsplit-proj.pgf}
  \begin{tikzpicture}
    \node[anchor=south west] (Im) at (0,0) {\includepgfgraphics[width=0.77\linewidth]{img/families/bulge1.5c/discsplit-cr.pgf}};
    \begin{scope}[x={(Im.south east)},y={(Im.north west)}]
      \draw[thick] (0.49,0.24) --+ (0,0.2);
    \end{scope}
  \end{tikzpicture}
...
\begin{figure}[htpb]
  ...
  \begin{tikzpicture}[
    ...
    \begin{scope}[x={(FxFz.south east)},y={(FxFz.north west)}]
      \coordinate (origin) at (0.107,0.111);
      \coordinate (x1y2) at (0.87,0.97);
      \draw[very thick,red!30!black] let \p1 = ($ (x1y2) - (origin) $) in 
      (origin)  -- node [pos=0.07, above=8pt] {$2.25$} + (0.89*\x1, \y1);
      \draw[very thick,red!30!black,dashed] (origin) -- node [pos=0.25,below] {$2$} (x1y2);
    \end{scope}
  \end{tikzpicture}
...
соответственно\note{Подобные обозначения обусловлены тем, что оба семейства вносят вклад в барлинзу (bl). 
  На двумерной карте $ω_x$ -- $ω_R$ одно лежит под баром (u), другое над ним (o).}.
