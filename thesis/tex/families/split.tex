...
$\text{bl}_{\text{o}}$ разбиваются на две группы: с $ω_x + ω_y \approx ω_R$ и с $ω_x \approx ω_y$. Первое из них
было отмечено в работе \cite{gajda2016} как семейство не поддерживающее бар. Но при этом авторы включают в него и
...
<<перешеек>>, по середине которого и проходит демаркационная линия.}, однако для них вытянутые семейства содержат
